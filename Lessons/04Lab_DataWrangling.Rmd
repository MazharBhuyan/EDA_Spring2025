...
Other_Way_NTL.phys.data.1999 <- NTL.phys.data %>% 
                                filter(year4 == 1999)
# the pipe operator alsoe worked fine
...
#NTL.phys.data.Tsdlake <- 
  #filter(NTL.phys.data, lakename == "Tuesday Lake" , year4 == c(1990:1999)) %in% 
  #filter(NTL.phys.data, year4 %in% c(1990:1999))

NTL.phys.data.Tsdlake <- 
  filter(NTL.phys.data, lakename == "Tuesday Lake" , year4 %in% c(1990:1999))

#Other_way_pipe.NTL <- NTL.phys.data %>%
  #filter(lakename == "Tuesday Lake", year4 == c(1990:1999))
```
...
#NTL.phys.data.Tsdlake.90.99.july <- NTL.phys.data %>%
...
Ex4 <- NTL.phys.data.Tsdlake.90.99.july %>%
  filter(depth <0.25) %>%
  drop_na(temperature_C) %>% #filter out missing obs
